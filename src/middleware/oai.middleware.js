...
//var rawData = buff;
oai.makeCall = function(numberType,callNumber,callback) {
  var flag = 0;
  winston.info(callNumber);
  // Create connection
  ...
  client.on('data',function(data) {
    if(!(Buffer.compare(data,inPacket1))) // if get Frame 2
    {
      flag=1;
      console.log('CASE 1');
      writeData(client,outPacket2);  //  Send Frame 3
      writeData(client,outPacket3); //  Send Frame 4
    }
    if(!(Buffer.compare(data,inPacket2))&& flag==1)  //  if get Frame 5
    {
      flag=2;
      console.log('CASE 2');
      writeData(client,outPacket4);  //  Send Frame 6
    }
    if(!(Buffer.compare(data,inPacket4))&& flag==2)  // if get Frame 8
    {
      flag=3;
      console.log('CASE 3');
      writeData(client,outPacket5);  //  Send Frame 9
    }
    ...
    if(data.length == 39)//if(data[0]==inPacket5[0])  //else if(!(Buffer.compare(data,inPacket5)))  //  if get Frame 10
    {
      flag=4;
      console.log('CASE 4');
      writeData(client,outPacket6);  //  Send Frame 11
    }
    if(!(Buffer.compare(data,inPacket6))&& flag==4)  //  if get Frame  12
    {
      flag=5;
      console.log('CASE 5');
      writeData(client,outPacket7);  //  Send Frame 13
    }
    if(!(Buffer.compare(data,inPacket7))&& flag==5)  //  if get Frame 14
    {
      flag=6;
      console.log('CASE 6');
      //console.log("We should move to nect step");
      writeData(client,outPacket8);
    }
    //console.log(data);
    //console.log(data.length);
    if(flag==6)
    ...
    if(data[0] == inPacket9[0] &&
      ...
      console.log('Flag=6');
      console.log('CASE 7');
      writeData(client,outPacket9);
      writeData(client,outPacket10);
      flag=7;
      }
    }
    else if(flag==7)
